...
export async function getDenmarkUpdates(
  source: string,
  sourceConfig: DenmarkBondsourceConfig,
  logTab: number
) {
  try {
    let bonds_list = await getDenmarkBonds(source, sourceConfig, logTab + 1) // returns Array
    let chunks = tools.arrayToChunks(bonds_list, sourceConfig.get_bonds_details_chunck_size)
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      master_data = master_data.concat(await getDenmarkChunkBondDetails(source, chunks[i], logTab + 1))
    }
    lg(`END - ${tools.gFName(new Error())}`, logTab, 'info', source)
...
export async function getDenmarkBonds(source: string, sourceConfig: DenmarkBondsourceConfig, logTab: number) {
  try {
    let all_data = new Array()
    let end_reached = false
    let page_id = 0
    while (!end_reached) {
      lg(`Get Denmark bonds page : ${page_id}`, logTab + 1, 'info', source)
      let this_page_data = await getDenmarkBondsListInPage(source, sourceConfig, page_id, logTab + 1)
      all_data = all_data.concat(this_page_data)
      lg(`[${this_page_data.length}] Bonds in this page`, logTab + 1, 'info', source)
      page_id++
      if (this_page_data.length < 1 || page_id >= sourceConfig.page_limit) {
        end_reached = true
...
export async function getDenmarkBondsListInPage(
  source: string,
  sourceConfig: DenmarkBondsourceConfig,
  logTab: number,
  page_id: number
  ...
  try {
    lg(`START - getDenmarkBondsListInPage`, logTab, 'info', source)
    const size = sourceConfig.get_bonds_list_size
    page_id = page_id * size
    const URI = `http://www.nasdaqomxnordic.com/webproxy/DataFeedProxy.aspx`
    ...
    const response = await Ffetch.down(
      {
        ...
        headers: {
          ...
          Cookie:
            'JSESSIONID=C6BA4D934135AFB2747F4ABC36F8434B; NSC_MC_OBTEBRPNYOPSEJD_TTM=ffffffff09be0e8045525d5f4f58455e445a4a42378b; ak_bmsc=C3F301E87012F9CD1F4C739AFF99AFC4~000000000000000000000000000000~; ASP.NET_SessionId=shedksqhlau2a155vsox1ybv; bm_sv=8CE64145843B5095E65288B8F246647A~~1; bm_mi=D664634B3376F170632CC42D8D4F4E00~~1',
        },
      },
      logTab + 1
    )
    ...
    lg(`END - getDenmarkBondsListInPage`, logTab, 'info', source)
    return isins_labels_array
  } catch (err) {
...
export async function getDenmarkChunkBondDetails(source, chunk, logTab) {
  try {
    let this_chunck_data = await Promise.all(
      chunk.map(async (bond, index) => {
        let one_bond_details = await getDenmarkOneBondDetails(source, bond.isin, bond.label, logTab + 1)
        return {
          isin: bond.isin,
          name: bond.label,
...
export async function getDenmarkOneBondDetails(source, isin, label, logTab) {
  try {
    ...
    // let response = await tor.downloadUrlUsingTor("Denmark_one_bond", link, sourceConfig[source].defheaders, "POST", dataString, false);
