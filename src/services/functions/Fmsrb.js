...
async function getMSRBUpdates(source, date, log_tab) {
  try {
    ...
    const globalFiles = await generateGlobalFiles(source, allNotices, date, log_tab + 1)
...
async function getAllCategoriesNoticesByDate(source, date, log_tab) {
  try {
    ...
    const chunks = tools.arrayToChunks(
...
async function getNoticesByCategoryAndDate(source, category, date, log_tab) {
  try {
    ...
    const formattedDate = moment(date, 'YYYYMMDD').format('MM/DD/YYYY')
    const searchReqBody = _.cloneDeep(gfinalConfig[source].searchReqBody)
    ...
    const notices = CDDocuments.map(notice => {
...
async function getAllNoticesPDFs(source, allNotices, date, log_tab) {
  try {
    ...
    const chunks = tools.arrayToChunks(allNotices, gfinalConfig[source].chunck_size, log_tab + 1, source)
    let all_data = []
    for (let i = 0; i < chunks.length; i++) {
...
async function getOneNoticePDFs(source, notice, date, log_tab) {
  try {
    const notices = [] //will include merged an unmerged PDFs
    if (notice.mainUrl.includes('P21237353')) return notices
    const page = await Ffetch.down(
      {
        source,
        method: 'GET',
        url: notice.mainUrl,
    ...
    if (aTag.attr('help') === undefined) {
      ...
      const help = `<html>${aTag.attr('help')}</html>`
    ...
    notice.PDFs = await Promise.all(
      PDFs.map(async pdf => {
        const { shortName, res } = await Ffetch.downloadBinaryFile(
          source,
          {
            url: pdf.fileUrl,
            prefix: `${date}-MSRB`,
    ...
    if (notice.PDFs.length > 1) {
      ...
      if (output.unmergedFiles.length > 0) {
        output.unmergedFiles.map(unmergedFile => {
          clg(unmergedFile)
          const tempNotice = _.cloneDeep(notice)
  ...
  try {
    ...
    const globalFiles = await Promise.all(
      gfinalConfig[source].categories.map(async cat => {
        const thisCategoryNotices = allNotices.filter(notice => notice.id === cat.id && !notice.corrupt)
        if (thisCategoryNotices.length > 0) {
          ...
          const thisGlobalFileSize = thisCategoryNotices.reduce((acc, tmp) => acc + tmp.size, 0)
          return {
            date: date,
            id: cat.id,
            label: cat.label,
