...
async function getFinancialYearEndDate(source, html, log_tab) {
  ...
  const yearEndRow = trs.find(row => row.label === 'Fiscal year end')
  const yearEndMonth = yearEndRow ? moment(yearEndRow.value, 'MM/YYYY').format('MM') : ''
  // let yearend_month =
  //   yearEndRow
  //     ? 
  //       // yearEndRow.value.replace(/\.\d+/g, '')
  //       yearEndRow.value
  //     : ''
  // yearend_month = yearend_month !== '' ? moment(yearend_month, 'M', true).format('MMMM') : ''
  // let yearend = yearend_year !== '' ? `${yearend_month} ${yearend_year}` : ''
  let yearend = yearEndMonth ? `${moment(yearEndRow.value, 'DD/YYYY').endOf('month').format('DD')}/${yearEndMonth}` : ''
  // let yearend = yearend_year !== '' ? `${yearend_month} ${yearend_year}` : ''
  return yearend
}
